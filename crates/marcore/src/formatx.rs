...
macro_rules! fmt_slice_debug {
    ($xs:expr) => {{ $crate::formatx::fmt_slice_compact($xs, None) }};

    ($xs:expr, ALL) => {{ $crate::formatx::fmt_slice_compact($xs, Some(usize::MAX)) }};

    ($xs:expr, $n:expr) => {{ $crate::formatx::fmt_slice_compact($xs, Some($n)) }};
}
